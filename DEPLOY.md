...
**Vercel live site:** A git-triggered Vercel build already re-fetches FPL (`fetch-league-if-needed.mjs` skips only when `CI=true`, which GitHub Actions sets; Vercel sets `CI=1`). Scheduled Actions still deploy **GitHub Pages only**. The public site at **tclot.vercel.app** does not rebuild on cron unless you add a **Deploy Hook**: Vercel → Project → Settings → Git → Deploy Hooks → create one, then save the URL as repository (or `github-pages` Environment) secret **`VERCEL_DEPLOY_HOOK`**. The deploy workflow POSTs that hook after every successful Pages deploy. Without that secret, waiver points and rich recaps stay on the last git-push snapshot until someone merges.
